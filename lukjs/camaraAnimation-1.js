...
let capture;
let pixelSize = 8;   // większa wartość = większe piksele = szybciej
let cols;
let rows;
let gammaZ = 0.5;
let t = 0;
...
function setup() {
  ...
  c.parent("canvasWrap");       // attach canvas into the div

  frameRate(60);
  colorMode(RGBA, 255, 0);
  // clear(); // makes first frame transparent
  ...
  initCodeLines(); // TU była przyczyna błędu – funkcja musi być zdefiniowana niżej
}

function draw() {
  // 1) tło i pikselowa ściana z kamery
  drawSoftDarkBackgroundZ();
  drawZordonWall();

  // 2) zielony kod na wierzchu
  // drawCodeOverlay();
}

// --- główny efekt pikselowej ściany ---------------------------------------
function drawZordonWall() {
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      // pobranie jasności z kamery (lustrzane odbicie w poziomie)
      ...
      // „głębokość” i rozmiar prostokąta z jasności
      ...
      // jeśli chcesz jeszcze szybciej, możesz ten fragment zakomentować
      let d = dist(cx, cy, mouseX, mouseY);
      if (d < 150) {
        let force = (150 - d) / 150;
        let ang = atan2(cy - mouseY, cx - mouseX);
        cx += cos(ang) * force * 28;
        cy += sin(ang) * force * 28;
      }

      // kolor: pomarańcz–czarny / pomarańcz–biały
      let colR, colG, colB;
...
// tło z lekką poświatą (tu też możesz przyspieszyć, zmieniając krok r)
...
function drawCodeOverlay() {
  ...
  textSize(18);
  textAlign(LEFT, TOP);
  fill(0, 255, 70); // zielony

  // aktualizacja pisania (tempo w znakach/sek)
  if (!typingFinished) {
  ...
  // rysowanie: baza przy samym dole
  let baseY = height - codeMarginBottom - codeLineHeight;

  // idziemy od dołu w górę po liniach, ale logicznie od końca do początku
